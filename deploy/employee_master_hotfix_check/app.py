...
def generate_worker_code(db, worker_category, subcontractor_id=None):
    if worker_category == "Sub Contractor Staff" and subcontractor_id:
        ...
        if sub and sub["subcontractor_name"]:
            prefix = sub["subcontractor_name"][:2].upper()
            row = db.execute(
                "SELECT COUNT(*) AS cnt FROM workers WHERE subcontractor_id=?",
                (subcontractor_id,),
            ).fetchone()
            return f"{prefix}{int(row['cnt']) + 101}"
    row = db.execute("SELECT COUNT(*) AS cnt FROM workers").fetchone()
    return f"WRK{int(row['cnt']) + 101}"
...
NAV_ITEMS = [
    {"endpoint": "dashboard", "label": "Dashboard", "icon": "fa-gauge-high", "active_endpoints": ["dashboard", "dashboard_choice_b"]},
    {"endpoint": "projects", "label": "Projects", "icon": "fa-layer-group", "active_endpoints": ["projects", "clients"]},
    {"endpoint": "salary", "label": "HR & Payroll", "icon": "fa-money-bill-wave", "active_endpoints": ["salary"]},
    {"endpoint": "staff", "label": "Employees", "icon": "fa-users", "active_endpoints": ["staff", "workers"]},
    {"endpoint": "attendance", "label": "Attendance", "icon": "fa-calendar-check", "active_endpoints": ["attendance"]},
    {"endpoint": "timesheet", "label": "Timesheet", "icon": "fa-clock", "active_endpoints": ["timesheet"]},
    {"endpoint": "petty_cash", "label": "Petty Cash", "icon": "fa-wallet", "active_endpoints": ["petty_cash"]},
    {"endpoint": "head_office_expenses", "label": "Accounts", "icon": "fa-landmark", "active_endpoints": ["head_office_expenses", "project_expenses", "accounts"]},
    {"endpoint": "material_request", "label": "Store", "icon": "fa-warehouse", "active_endpoints": ["material_request", "store"]},
    {"endpoint": "purchase_request", "label": "Purchase", "icon": "fa-cart-shopping", "active_endpoints": ["purchase_request", "purchase"]},
    {"endpoint": "inventory", "label": "Inventory", "icon": "fa-boxes-stacked", "active_endpoints": ["inventory"]},
    {"endpoint": "subcontract_request", "label": "Subcontract", "icon": "fa-people-group", "active_endpoints": ["subcontract_request", "subcontractors"]},
    {"endpoint": "approvals", "label": "Approvals", "icon": "fa-clipboard-check", "active_endpoints": ["approvals", "approval_action"]},
    {"endpoint": "notifications", "label": "Notifications", "icon": "fa-bell", "active_endpoints": ["notifications"]},
    {"endpoint": "reports", "label": "Reports", "icon": "fa-file-excel", "active_endpoints": ["reports"]},
    {"endpoint": "settings", "label": "Settings", "icon": "fa-gear", "active_endpoints": ["settings", "workflow_settings", "workflow_matrix", "user_settings"]},
]
